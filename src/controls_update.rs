...
pub fn toggle_breakpoint(application_state: &mut ApplicationState, bp: usize) -> Task<Message> {
    if application_state.control_related_data.breakpoint == Some(bp) {
        application_state.control_related_data.breakpoint = None;
    } else {
        application_state.control_related_data.breakpoint = Some(bp)
    }
    Task::none()
}

pub fn scroll_to_pc(application_state: &mut ApplicationState) -> Task<Message> {
    if !application_state.control_related_data.auto_scroll_pc {
        return Task::none();
    };
    ...
    let position = normalised_pc as f32
        / (application_state
            .emulator_related_data
            .current_program
            .len()
...
pub fn toggle_auto_scroll_to_pc(application_state: &mut ApplicationState) -> Task<Message> {
    application_state.control_related_data.auto_scroll_pc =
        !application_state.control_related_data.auto_scroll_pc;
